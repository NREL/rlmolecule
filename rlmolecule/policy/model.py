...
def policy_model(
    ...
    :param num_heads: Number of global state attention heads. Must be a factor of
        `features`
    ...
    atom_class = layers.Input(
        shape=[None], dtype=input_dtype, name="atom"
    )  # batch_size, num_atoms
    bond_class = layers.Input(
        shape=[None], dtype=input_dtype, name="bond"
    )  # batch_size, num_bonds
    connectivity = layers.Input(
        shape=[None, 2], dtype=input_dtype, name="connectivity"
