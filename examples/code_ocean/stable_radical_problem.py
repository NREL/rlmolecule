...
from bde_utils import prepare_for_bde
from stable_radical_molecule_state import (
...
class StableRadOptProblem(MoleculeTFAlphaZeroProblem):
    ...
    def calc_bde(self, state: MoleculeState):
        ...
        pred_bdes = alfabet_model.predict(
            [bde_inputs.mol_smiles], drop_duplicates=False
        )
        pred_bdes = pred_bdes.set_index("bond_index").bde_pred

        bde_radical = pred_bdes.loc[bde_inputs.bond_index]

        if len(bde_inputs.other_h_bonds) == 0:
            bde_diff = 30.0  # Just an arbitrary large number

        else:
            other_h_bdes = pred_bdes.loc[bde_inputs.other_h_bonds]
            bde_diff = (other_h_bdes - bde_radical).min()

        return bde_radical, bde_diff


def construct_problem(run_config: RunConfig, **kwargs):
    prob_config = run_config.problem_config

    stability_model = tf.keras.models.load_model(
        prob_config.get("stability_model"), compile=False
    )
    redox_model = tf.keras.models.load_model(
        prob_config.get("redox_model"), compile=False
    )
