...
WORKING_DIR="/projects/rlmolecule/$USER/crystal_vol/${run_id}"
mkdir -p $WORKING_DIR
...
sed -i "s/crystal_vol_example/$run_id/" $SCRIPT_CONFIG
...
python -u optimize_crystal_volume.py --train-policy --config $SCRIPT_CONFIG
EOF
...
module load cudnn/8.1.1/cuda-11.2
conda activate crystals
python -u optimize_crystal_volume.py --rollout --config $SCRIPT_CONFIG
EOF
