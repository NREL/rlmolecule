...
class HallwayAlphaZeroProblem(AlphaZeroProblem):
    ...
    def _get_network_inputs_from_serialized_parent(self, serialized_parent: tf.Tensor) -> ({}, {}):
    ...
    def train_policy_model(self,
        ...
        self.policy_model.compile(optimizer=tf.keras.optimizers.Adam(lr),
                                  loss=[tf.keras.losses.BinaryCrossentropy(from_logits=True),
                                        KLWithLogits()])

        logger.info("Policy trainer: starting training")

        return self.policy_model.fit(dataset,
