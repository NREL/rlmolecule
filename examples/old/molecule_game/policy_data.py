...
def parse_binary_data(binary_data, reward):
    ...
    return (parsed_data['atom'], parsed_data['bond'], parsed_data['connectivity'], int(reward), visit_probs)
...
def parse_data_tf(binary_data, reward):
    ...
    atom, bond, connectivity, reward, visit_probs = tf.py_function(
        parse_binary_data, inp=[binary_data, reward], Tout=[tf.int64, tf.int64, tf.int64, tf.int64, tf.float32])
    ...
    return ({'atom': atom, 'bond': bond, 'connectivity': connectivity}, (reward, visit_probs))
